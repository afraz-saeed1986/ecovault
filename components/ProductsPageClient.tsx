...
export default function Home() {
  ...
  const fetchPage = useCallback(
    async (pageNumber: number, category: string, reset: boolean = false) => {
      try {
        const url = `/api/products?page=${pageNumber}&limit=6${
  ...
  useEffect(() => {
    if (inView && !loadingMore && !loading && page < totalPages) {
      ...
      setPage(nextPage);
      fetchPage(nextPage, selectedCategory).finally(() =>
  ...
  // جستجو
  const fuse = useMemo(
    () => new Fuse(products, { keys: ["name", "description"], threshold: 0.3 }),
    [products]
  );
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <aside className="hidden lg:block w-64 flex-shrink-0">
          ...
          <button
            ...
            className={`w-full text-left dark:text-eco-light px-4 py-2.5 rounded-lg flex items-center gap-2 transition-all mb-2 ${
              selectedCategory === "all"
                ? "bg-eco-green text-white shadow-md"
                : "hover:bg-eco-green hover:text-white"
            }`}
          >
            All Products
          </button>
            ...
            {allCategories.map((cat) => (
              <li key={cat.id}>
                <button
                  ...
                  className={`w-full text-left px-4 py-2 rounded-lg dark:text-eco-light flex items-center gap-2 transition-all ${
                    selectedCategory === cat.id
                      ? "bg-eco-green text-white shadow-md"
                      : "hover:bg-eco-green hover:text-white"
                  }`}
