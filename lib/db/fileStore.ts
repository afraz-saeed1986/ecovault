...
export async function upsert<T extends { id?: string | number }>(name: string, item: T): Promise<T> {
  ...
  try {
    ...
    const existing = collectionRaw as T[];

    let nid = item.id ? normalizeId(item.id) : undefined;
    if (!nid) {
      // const ids = existing.map((it) => (it as any)["id"]).filter(Boolean);
    ...
    // const idx = existing.findIndex((it) => String((it as any)["id"]) === String(item.id));
